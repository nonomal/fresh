...
impl Editor {
    ...
    pub fn apply_event_to_active_buffer(&mut self, event: &Event) {
        ...
        // Borrow cursors from SplitViewState (sole source of truth) and state from buffers
        {
            ...
            let cursors = &mut self.split_view_states.get_mut(&split_id).unwrap()
                .keyed_states.get_mut(&active_buf).unwrap().cursors;
    ...
    pub fn apply_events_as_bulk_edit(
        ...
        let old_cursors: Vec<(CursorId, usize, Option<usize>)> = self
            .split_view_states.get(&split_id).unwrap()
            .keyed_states.get(&active_buf).unwrap()
            .cursors
            .iter()
        ...
        self.buffers.get_mut(&active_buf).unwrap().highlighter.invalidate_all();
    ...
    fn update_plugin_state_snapshot(&mut self) {
        ...
        if let Some(snapshot_handle) = self.plugin_manager.state_snapshot_handle() {
            ...
            if let Some(active_vs) = self.split_view_states.get(&self.split_manager.active_split()) {
        ...
        match command {
            ...
            PluginCommand::GetTextPropertiesAtCursor { buffer_id } => {
                ...
                if let Some(state) = self.buffers.get(&buffer_id) {
                    let cursor_pos = self.split_view_states.values()
