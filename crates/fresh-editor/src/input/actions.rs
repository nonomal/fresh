...
fn convert_block_selection_to_cursors(state: &mut EditorState, cursors: &mut Cursors) -> Vec<Event> {
...
fn transform_case<F>(state: &mut EditorState, cursors: &mut Cursors, events: &mut Vec<Event>, transform: F)
where
    F: Fn(&str) -> String,
{
...
pub fn action_to_events(
    ...
    match action {
        ...
        Action::TransposeChars => {
            ...
            let cursor_positions: Vec<_> = cursors
                .iter()
...
mod tests {
    ...
    fn test_backspace_deletes_newline() {
        ...
        let events =
            action_to_events(&mut state, &mut cursors, Action::DeleteBackward, 4, false, 80, 24).unwrap();
    ...
    fn test_move_down_basic() {
        ...
        let events = action_to_events(&mut state, &mut cursors, Action::MoveDown, 4, false, 80, 24).unwrap();
    ...
    fn test_move_line_up_without_trailing_newline() {
        ...
        let events = action_to_events(&mut state, &mut cursors, Action::MoveLineUp, 4, false, 80, 24).unwrap();
        for event in events {
            state.apply(&mut cursors, &event);
        }
    ...
    fn test_move_line_down_without_trailing_newline() {
        ...
        let events = action_to_events(&mut state, &mut cursors, Action::MoveLineDown, 4, false, 80, 24).unwrap();
        for event in events {
            state.apply(&mut cursors, &event);
        }
    ...
    fn test_move_up_basic() {
        ...
        let events = action_to_events(&mut state, &mut cursors, Action::MoveUp, 4, false, 80, 24).unwrap();
        assert_eq!(events.len(), 1);
    ...
    fn test_move_line_end_positioning() {
        ...
        // Move to line end
        let events = action_to_events(&mut state, &mut cursors, Action::MoveLineEnd, 4, false, 80, 24).unwrap();
        for event in events {
    ...
    fn test_move_line_start_from_eof() {
        ...
        // Move to line start
        let events = action_to_events(&mut state, &mut cursors, Action::MoveLineStart, 4, false, 80, 24).unwrap();
        for event in events {
    ...
    fn test_bracket_auto_close_parenthesis() {
        ...
        let events =
            action_to_events(&mut state, &mut cursors, Action::InsertChar('('), 4, true, 80, 24).unwrap();
    ...
    fn test_bracket_auto_close_disabled_when_auto_indent_false() {
        ...
        let events =
            action_to_events(&mut state, &mut cursors, Action::InsertChar('('), 4, false, 80, 24).unwrap();
    ...
    fn test_bracket_auto_close_multiple_cursors_with_skip_over() {
        ...
        // Type 'f'
        let events =
            action_to_events(&mut state, &mut cursors, Action::InsertChar('f'), 4, true, 80, 24).unwrap();
        for event in events {
            state.apply(&mut cursors, &event);
        }
        ...
        // Type 'o'
        let events =
            action_to_events(&mut state, &mut cursors, Action::InsertChar('o'), 4, true, 80, 24).unwrap();
        for event in events {
            state.apply(&mut cursors, &event);
        }
    ...
    fn test_bracket_auto_close_three_cursors_with_skip_over() {
        ...
        for ch in ['f', 'o', 'o'] {
            let events =
                action_to_events(&mut state, &mut cursors, Action::InsertChar(ch), 4, true, 80, 24).unwrap();
            for event in events {
                state.apply(&mut cursors, &event);
            }
        }
    ...
    fn test_auto_pair_deletion_parenthesis() {
        ...
        let events = action_to_events(&mut state, &mut cursors, Action::DeleteBackward, 4, true, 80, 24).unwrap();
